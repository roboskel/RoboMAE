...
function [CM,LabelMapping,n_table,Matching,Cost] = ExtractTablefromLabels( speakerConfidence,probsSmooth,speakerLabelsNew,LabelsS,Flag)

VidLabels = num2cell(speakerLabelsNew);
% VidLabels = makeLabelsSuccessive(VidLabels);
AudLabels = num2cell(LabelsS);
% AudLabels = makeLabelsSuccessive(AudLabels);    
if Flag==1 % We don't deal with the confidences
    n_table = contigencyTableZ(VidLabels, AudLabels,1);   
else
    n_table = contigencyWeightedTable(VidLabels, AudLabels,1,speakerConfidence,probsSmooth);
end
