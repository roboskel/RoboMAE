...
function add_paths(matlabHome)

% define the path seperator depending on the OS
if (ispc)
	sSep = '\';
else
	sSep = '/';
...
addpath([matlabHome sSep 'matlabAudioAnalysis']);
addpath([matlabHome sSep 'speakerDiarization' sSep 'clustTool']);
addpath([matlabHome sSep 'XML']);
