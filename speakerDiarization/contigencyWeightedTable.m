...
function n = contigencyWeightedTable(results, realLabels, ignoreMultiLabels,speakerConfidence,probsSmooth)
    ...
    if (~ignoreMultiLabels)
        for (k1=1:length(results{i})) % for all labels of sample i
            for (k2=1:length(realLabels{i})) % for all real labels of sample j
                if results{i}(k1) ~=0 && realLabels{i}(k2)~=0
                    n(results{i}(k1), realLabels{i}(k2)) = n(results{i}(k1), realLabels{i}(k2)) + 0.5*(speakerConfidence(results{i}(1),i) + probsSmooth(realLabels{i}(1),i));           
                end
            end
        end
    else
        if ((length(results{i})==1) && (length(realLabels{i})==1))
            if results{i}(1) ~=0 && realLabels{i}(1)~=0
                n(results{i}(1), realLabels{i}(1)) = n(results{i}(1), realLabels{i}(1)) + 0.5*(speakerConfidence(results{i}(1),i)+probsSmooth(realLabels{i}(1),i));
            end
        end
    end
