...
function [Times, Labels] = clusteResults2LabelsAndTimes(results, speechSegmentsIndeces, totalWavLength, timeResolution)

Time = 0;
count = 0;
while (Time+timeResolution<totalWavLength)
    count = count + 1;
    Times(count) = Time;
    Indx = find(speechSegmentsIndeces==Time);
    if (isempty(Indx))
        Labels(count) = 0;
    else
        Labels(count) = results(Indx);
    end
